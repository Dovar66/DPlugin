...
public class ArscEditor extends AssetEditor {
    ...
    //buildToolVersion在24.0.x之后是56,而28.0.x之后是64，后续升级gradle需要注意
     ...
     * @return
     */
    def slice(int pp, Map idMaps, Map libRefTable, List<PluginResourceEntry> retainedTypes) {
        def t = readTable()
        ...
        if (t.typeList.specs.size() == 0) {
            Log.i 'ArscEditor.slice', "\t -- There was no res."
            close()
            return
        }

        if (retainedTypes.size() == 0) {
            Log.i("ArscEditor.slice", "retainedTypeIds.size()==0")
            close()
            return
        }
        ...
        if (retainedTypes[0].id == Aapt.ID_NO_ATTR) { // attr type id is always at first
            ...
            Log.i("ArscEditor.slice", "\t -- There was no attr.")
        }
        ...
        retainedTypes.each {
            ...
            Log.i 'ProcessResourcesHooker', "typeList.specs${it.id} <<${specIndex}"
            def ts = t.typeList.specs[specIndex]
            List<PluginResourceSubEntry> es = it.entries
