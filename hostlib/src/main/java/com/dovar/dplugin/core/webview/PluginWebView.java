package com.dovar.dplugin.core.webview;
...
/**
 * 插件中的WebView并不能保证一定能读取主程序和插件内的assets中html等资源文件，
 * 其中，Android API < 19 只能读取插件assets中的html文件，而在 Android API >= 19 只能读取主程序assets中的html文件。
 * <p>
 * 其具体原因是：在API 19开始，webview内核由WebKit过渡到了Chromium，其内部的getAssets方法的Context由mContext改ApplicationContext。
 * 而API26之后WebView开始使用独立进程渲染，通过当前应用的ApplicationId，反查已安装应用的apk路径
 * <p>
 * 暂未支持新协议：file:///android_res
 */
public class PluginWebView extends WebView {
    ...
    @TargetApi(21)
    public PluginWebView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        init(context);
    }

    public PluginWebView(Context context, AttributeSet attrs, int defStyleAttr, boolean privateBrowsing) {
